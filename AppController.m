 ...
 + (void) initialize
{
	...
	if ( self == [AppController class] && initialized == NO)
	{
		...
		//	}
			...
			[[NSUserDefaults standardUserDefaults] registerDefaults: [AppController getDefaults]];
...
//			[NSTimer scheduledTimerWithTimeInterval:2.0 target:splashController selector:@selector(windowShouldClose:) userInfo:nil repeats:0]; 
	...
	{
		if(![hostName isEqualToString: @"lavimarch.hcuge.ch"] && ![hostName isEqualToString: @"drdd-mc19.hcuge.ch"] && ![hostName isEqualToString: @"uin-mc04.hcuge.ch"])
		{
