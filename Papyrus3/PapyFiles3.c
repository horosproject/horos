							...
							if ((theErr = Papy3GotoGroupNb (theFileNb, 0x0008)) >= 0)
							{
								if ((theErr = Papy3GroupRead (theFileNb, &theGroupP)) >= 0)
								{
									...
									if (theValP != NULL)
									{
										...
										if( strncmp( "1.2.840.10008.5.1.4.1.1.4.2", gSOPClassUID[ theFileNb], strlen( "1.2.840.10008.5.1.4.1.1.4.2")) == 0) // Spectro
											isImage = 0;
  	 ...
  	 theLastElemNb <= theTmpElemNb)
  {
    ...
    if (gArrTransfSyntax [inFileNb] == LITTLE_ENDIAN_EXPL || gArrTransfSyntax [inFileNb] == BIG_ENDIAN_EXPL || theTmpGrNb == 0x0002)
    {
      ...
      else /* other VRs */
      {
		...
		theTmpElemL = UIntToHost( inFileNb, theCharP);
