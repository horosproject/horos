...
int intersect3D_SegmentPlane2( float *P0, float *P1, float *Pnormal, float *Ppoint, float* resultPt )
{
    float    u[ 3];
	float    w[ 3];
...
//		if( )
		[self get3DPixelUnder2DPositionX:_mouseLocStart.x Y:_mouseLocStart.y pixel:pix position:pos value:&value];
		{
...
//			vPos[ 2] = [fObject originZ];
			...
			if( intersect3D_SegmentPlane2( point2, point1, planeVector, point, resultPt ))
			{
				float	tempPoint3D[ 3];
				long	ptInt[ 3];
				long	roiID;
...
-(void) setOpacity:(NSArray*) array
{
	...
	if( [array count] > 0)
	{
		...
		pt.x -=1000;
		if(pt.x != 0) opacityTransferFunction->AddPoint(0 +start, 1);
//		else NSLog(@"start point");
	}
	else opacityTransferFunction->AddPoint(0 +start, 1);
	
	for( i = 0; i < [array count]; i++)
	{
		pt = NSPointFromString( [array objectAtIndex: i]);
		pt.x -= 1000;
		opacityTransferFunction->AddPoint(start + (pt.x / 256.0) * (end - start), 1);
	}
...
- (BOOL) get3DPixelUnder2DPositionX:(float) x Y:(float) y pixel: (long*) pix position:(float*) position value:(float*) val maxOpacity: (float) maxOpacity minValue: (float) minValue
{
	...
	for( p = 0; p < stackMax; p++)
	{
		...
		if( intersect3D_SegmentPlane2(point2, point1, planeVector, currentPoint, resultPt))
		{
			// Convert this 3D point to 2D point projected in the plane
			float tempPoint3D[3];
...
- (void)connect2SpaceNavigator;
{
	...
	if(InstallConnexionHandlers != NULL)
	{
		...
		error = InstallConnexionHandlers(VRSpaceNavigatorMessageHandler2, nil, nil);
...
void VRSpaceNavigatorMessageHandler2(io_connect_t connection, natural_t messageType, void *messageArgument)
{
