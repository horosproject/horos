...
- (void) exportDICOMFile:(id) sender
{
	...
	if ([sPanel runModalForDirectory:0L file:0L types:0L] == NSFileHandlingPanelOKButton)
	{
		...
		for( i = 0; i < [filesToExport count]; i++)
		{
			...
			tempPath = [tempPath stringByAppendingFormat:@" - %@", [curImage valueForKeyPath: @"series.id"]];
...
//	for( i = 0; i < [filesToSend count]; i++)		// Sending all files with a single storescu is MUCH faster !
	{
		...
		NSLog(@"destinationServer: %@", [destinationServer description]);
		
		if ([destinationServer isMemberOfClass:[NSNetService class]]){
			[theArguments addObject:[destinationServer hostName]];
			[theArguments addObject:[NSString stringWithFormat:@"%d", [[DCMNetServiceDelegate sharedNetServiceDelegate] portForNetService:destinationServer]]];
		}
		else{
			[theArguments addObject:[destinationServer objectForKey:@"Address"]];
			[theArguments addObject:[destinationServer objectForKey:@"Port"]];
		}
		
	//	[theArguments addObject:[filesToSend objectAtIndex: i]];
		
		[theArguments addObjectsFromArray: filesToSend];
		
		[theArguments addObject:@"-aec"];
		if ([destinationServer isMemberOfClass:[NSNetService class]])
			[theArguments addObject:[destinationServer name]];
		else			
			[theArguments addObject:[destinationServer objectForKey:@"AETitle"]];
		[theArguments addObject:@"-aet"];
		[theArguments addObject:AETitle];
		
		switch( [syntaxListOffis indexOfSelectedItem])
		{
			case 0:		[theArguments addObject:@"-xi"];	NSLog(@"-xi");		break;
			case 1:		[theArguments addObject:@"-xe"];	NSLog(@"-xe");		break;
			case 2:		[theArguments addObject:@"-xb"];	NSLog(@"-xb");		break;
			case 3:		[theArguments addObject:@"-xs"];	NSLog(@"-xs");		break;
			case 4:		[theArguments addObject:@"-xr"];	NSLog(@"-xr");		break;
		}
		...
		[waitSendWindow setCancel:YES];
		// launch traceroute
		[theTask launch];
		while( [theTask isRunning] == YES && [waitSendWindow aborted] == NO)
		{
			[NSThread sleepUntilDate:[NSDate dateWithTimeIntervalSinceNow:0.02]];
		}
		
		if( [waitSendWindow aborted] == NO)
		{
...
//		else i = [filesToSend count];
		...
		if ([destinationServer isMemberOfClass:[NSNetService class]])
			[info setObject:[destinationServer name] forKey:@"CalledAET"]; 
		else
			[info setObject:[destinationServer objectForKey:@"AETitle"] forKey:@"CalledAET"];
		[[NSNotificationCenter defaultCenter] postNotificationName:@"DICOMSendStatus" object:nil userInfo:info];
	}
	
	[destinationServer release];
	[filesToSend release];
	[pool release];
}
...
{
	NSAutoreleasePool   *pool=[[NSAutoreleasePool alloc] init];
	//id server = [destinationServer copy];
	//[destinationServer release];	
	NSMutableArray		*filesToSend = [files retain];
	...
	if ([transferSyntax isEqualToString:@"Explicit Little Endian"])
		ts = [DCMTransferSyntax ExplicitVRLittleEndianTransferSyntax];
	else if ([transferSyntax isEqualToString:@"JPEG 2000 Lossless"])
		ts = [DCMTransferSyntax JPEG2000LosslessTransferSyntax];
	else if ([transferSyntax isEqualToString:@"JPEG 2000 Lossy 10:1"]) {
		ts = [DCMTransferSyntax JPEG2000LossyTransferSyntax];
		compression = DCMHighQuality;
	}
	else if ([transferSyntax isEqualToString:@"JPEG 2000 Lossy 20:1"]) {
		ts = [DCMTransferSyntax JPEG2000LossyTransferSyntax];
		compression = DCMMediumQuality;
	}
	else if ([transferSyntax isEqualToString:@"JPEG 2000 Lossy 50:1"]) {
		ts = [DCMTransferSyntax JPEG2000LossyTransferSyntax];
		compression =  DCMLowQuality;
	}
	else if ([transferSyntax isEqualToString:@"JPEG Lossless"])
		ts = [DCMTransferSyntax JPEGLosslessTransferSyntax];
	else if ([transferSyntax isEqualToString:@"JPEG High Quality (9)"]) {
		ts = [DCMTransferSyntax JPEGExtendedTransferSyntax];
		compression = DCMHighQuality;
	}
	else if ([transferSyntax isEqualToString:@"JPEG Medium High Quality (8)"]) {
		ts = [DCMTransferSyntax JPEGExtendedTransferSyntax];
		compression =  DCMMediumQuality;
	}
	else if ([transferSyntax isEqualToString:@"JPEG Medium Quality (7)"]) {
		ts = [DCMTransferSyntax JPEGExtendedTransferSyntax];
		compression =  DCMLowQuality;
	}
	NSString *calledAET;
	...
	//bonjour
	if ([destinationServer isMemberOfClass:[NSNetService class]]){
		calledAET = [destinationServer name];
		hostname = [destinationServer hostName];
		destPort = [NSString stringWithFormat:@"%d", [[DCMNetServiceDelegate sharedNetServiceDelegate] portForNetService:destinationServer]];
		netService = destinationServer;		
		...
		calledAET = [destinationServer objectForKey:@"AETitle"];
		hostname = [destinationServer objectForKey:@"Address"];
		destPort = [destinationServer objectForKey:@"Port"];
		objects = [NSArray arrayWithObjects:filesToSend, 
						[NSNumber numberWithInt:compression],
						ts,
						AETitle, 
...
//	[server release];
	...
	[waitSendWindow close];
	[waitSendWindow release];
	waitSendWindow = 0L;
...
/*static NSArray	*objectsToSend = 0L;
...
- (IBAction) endSelectServer:(id) sender
{
	...
	if( [[DICOMSendTool selectedCell] tag] == 1) usePixelMed = YES;
	else usePixelMed = NO;
	
	[[NSUserDefaults standardUserDefaults] setInteger:[[DICOMSendTool selectedCell] tag] forKey:@"lastSenderEngine"];
	[[NSUserDefaults standardUserDefaults] setInteger:[[keyImageMatrix selectedCell] tag]  forKey:@"lastSendWhat"];
	
	[[NSUserDefaults standardUserDefaults] setInteger:[syntaxListOsiriX indexOfSelectedItem]  forKey:@"syntaxListOsiriX"];
	[[NSUserDefaults standardUserDefaults] setInteger:[syntaxListOffis indexOfSelectedItem]  forKey:@"syntaxListOffis"];
	
	[serverWindow orderOut:sender];
	
	[NSApp endSheet: serverWindow returnCode:[sender tag]];
	
	[objectsToSend autorelease];
	
	if( [sender tag])   //User clicks OK Button
    {
		if( waitSendWindow)
		{
			NSRunCriticalAlertPanel(NSLocalizedString(@"DICOM Send",nil),NSLocalizedString( @"OsiriX is already sending files. Wait or Abort current transfer.",nil),NSLocalizedString( @"OK",nil), nil, nil);
		}
		else
		{
			BOOL keyImagesOnly = NO, SCOnly = NO;
			if( [keyImageMatrix selectedRow] == 1) keyImagesOnly = YES;
			if( [keyImageMatrix selectedRow] == 2) SCOnly = YES;
			...
			if( files2Send != 0L && [files2Send count] > 0)
			{
				...
				if ([serverList indexOfSelectedItem] < [serversArray count])
					destinationServer = [[serversArray objectAtIndex:[serverList indexOfSelectedItem]] retain];
				//Bonjour
				else 
					destinationServer = [[[[DCMNetServiceDelegate sharedNetServiceDelegate] dicomServices] objectAtIndex:[serverList indexOfSelectedItem] - [serversArray count]] retain];
				
				waitSendWindow = [[Wait alloc] initWithString: NSLocalizedString(@"Sending files...",@"Sending files") :NO];
				
				[waitSendWindow showWindow:self];
				[[waitSendWindow progress] setMaxValue:[files2Send count]];
				
				if( usePixelMed)
				{
					[waitSendWindow setCancel:YES];
					transferSyntax = [[syntaxListOsiriX titleOfSelectedItem] retain];
					[NSThread detachNewThreadSelector: @selector(sendDICOMFiles:) toTarget:self withObject: files2Send];
				}
				else
				{
					[[waitSendWindow progress] setIndeterminate: YES];
					[waitSendWindow setElapsedString:@"working..."];
	...
	[[NSUserDefaults standardUserDefaults] setInteger:[serverList indexOfSelectedItem] forKey:@"lastSendServer"];
}
*/
...
- (void) selectServer: (NSArray*) objects
{
	...
	{
		if([objects count] == 1)
			[noImages setStringValue: [NSString stringWithFormat:NSLocalizedString(@"%d image", nil), [objects count]]];
		else if ([objects count] > 1)
			[noImages setStringValue: [NSString stringWithFormat:NSLocalizedString(@"%d images", nil), [objects count]]];
		
		objectsToSend = [objects retain];
		
		[serverList reloadData];
		[serverList selectItemAtIndex: [[NSUserDefaults standardUserDefaults] integerForKey:@"lastSendServer"]];
		
		[DICOMSendTool selectCellWithTag: [[NSUserDefaults standardUserDefaults] integerForKey:@"lastSenderEngine"]];
		[keyImageMatrix selectCellWithTag: [[NSUserDefaults standardUserDefaults] integerForKey:@"lastSendWhat"]];
		
		//[syntaxListOsiriX selectItemAtIndex: [[NSUserDefaults standardUserDefaults] integerForKey:@"syntaxListOsiriX"]];
		[syntaxListOffis selectItemAtIndex: [[NSUserDefaults standardUserDefaults] integerForKey:@"syntaxListOffis"]];
		
		[NSApp beginSheet: serverWindow modalForWindow:[NSApp mainWindow] modalDelegate:self didEndSelector:nil contextInfo:nil];
	}
	*/	
...
- (void)setSendMessage:(NSNotification *)note
{
	...
	NSString *calledAET = [info objectForKey:@"CalledAET"];
	BOOL sent = [[info objectForKey:@"Sent"] boolValue];
	NSString *message = @" ";
	if (sent == NO) {
		if ([info objectForKey:@"ErrorCount"]) {
			int errorCount = [[info objectForKey:@"ErrorCount"] intValue];
			message = [NSString stringWithFormat:NSLocalizedString(@"Sending %d/%d error: %d", nil), numberSent, count, errorCount];
		}
		else
			message = [NSString stringWithFormat:NSLocalizedString(@"Sending:%@ %d/%d", nil),calledAET, numberSent, count];
	}

//	[sendStatusField setStringValue:message];	// <- This field is already used to display the database content
//	[sendStatusField setNeedsDisplay:YES];
	
	if( waitSendWindow)
	{
		[waitSendWindow incrementBy:1];
	...
	if (date) {
		...
		if ([[activeSends allKeys] containsObject:date]){
			NSMutableDictionary *dictionary = [activeSends objectForKey:date];
			[dictionary setDictionary:info];
		}
		else {
			[activeSends setObject:info forKey:date];
			[sendLog addObject:info];
