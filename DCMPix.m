...
-(void) LoadTiff:(long) directory
{
	...
	//	NSLog(@"dir:%d", count);
		...
		NSLog( @"Bits Per Sample: %d, Samples Per Pixel: %d", bpp, spp);
...
//		}
		...
		width = height = 0;
		int spp = 1;
		if ([dcmObject attributeValueWithName:@"SamplesperPixel"])
			spp = [[dcmObject attributeValueWithName:@"SamplesperPixel"] intValue];
...
{
	int				elemType, depth, pixmin, pixmax, realwidth, realheight;
		...
		if ((err = Papy3GotoGroupNb (fileNb, 0x7FE0)) == 0)
		{
			...
			if ((err = Papy3GroupRead (fileNb, &theGroupP)) > 0) 
			{
				...
				if( fSetClut)
				{
					...
					//	if( depth != 8) NSLog(@"Palette with a non-8 bit image???");
...
-(float*) multiplyImages :(float*) input :(float*) subfImage
{
	...
	{
		#if __ppc__
		if( Altivec ) vmultiply(input, subfImage, result, i);
		else
		#endif
...
-(float*) subtractImages :(float*) input :(float*) subfImage
{
	...
	{
		#if __ppc__
		if( Altivec) vsubtract(input, subfImage, result, i);
		else
		#endif
    ...
    if( baseAddr)
    {
		...
		if( stackMode > 0 && stack >= 1)
		{
			...
			switch( stackMode)
			{
				...
				if( isRGB == NO)
				{
					...
					if( next < [pixArray count]  && next >= 0)
					{
						...
						if( fNext)
						{
							...
							if( Altivec)
							{
								if( stackMode == 2) vmax(fNext, fImage, fResult, height * width);
								else vmin(fNext, fImage, fResult, height * width);
							}
							else
							#endif
							{
						...
						for( i = 2; i < stack; i++)
						{
							...
							if( res < [pixArray count] && res >= 0)
							{
								...
								if( fNext)
								{
									...
									if( Altivec)
									{
										if( stackMode == 2) vmax(fResult, fNext, fResult, height * width);
										else vmin(fResult, fNext, fResult, height * width);
									}
									else
									#endif
									{
				...
				{
					unsigned char   *fResult, *fNext = 0L;
					...
					if( next < [pixArray count]  && next >= 0)
					{
						...
						if( fNext)
						{
							...
							if( Altivec)
							{
								if( stackMode == 2) vmax8(fNext, fImage, fResult, height * width);
								else vmin8(fNext, fImage, fResult, height * width);
							}
							else
							#endif
							{
							...
							if( res < [pixArray count])
							{
								...
								if( res < [pixArray count] && res >= 0)
								{
									...
									if( fNext)
									{
										...
										if( Altivec)
										{
											if( stackMode == 2) vmax8(fResult, fNext, fResult, height * width);
											else vmin8(fResult, fNext, fResult, height * width);
										}
										else
										#endif
										{
