...
//	OFCondition status = fileformat.loadFile([filePath UTF8String],  EXS_Unknown, EGL_noChange, DCM_MaxReadLength, ERM_autoDetect);
	...
	if (status.good())
	{
		...
		if( [self noLocalizer] && ([self containsString: @"LOCALIZER" inArray: imageTypeArray] || [serie rangeOfString:@"localizer" options:NSCaseInsensitiveSearch].location != NSNotFound || [serie rangeOfString:@"scout" options:NSCaseInsensitiveSearch].location != NSNotFound))
		{
			NSString	*n;
		...
		else if (([Modality isEqualToString:@"CR"] || [Modality isEqualToString:@"DR"] || [Modality isEqualToString:@"DX"] || [Modality isEqualToString:@"DX"] || [Modality  isEqualToString:@"RF"]) && [self combineProjectionSeries])
		{
