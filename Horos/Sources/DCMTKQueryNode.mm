		...
		@catch (NSException *e)
		{
            ...
            if (_abortAssociation == NO)
            {
                if( showErrorMessage == YES)
                    ...
                    [[AppController sharedAppController] displayUserNotification: NSLocalizedString(@"Query Failed (1)", nil) description: response name: @"autoquery"];
			}
    ...
    if (cond == EC_Normal)
	{
		if( rsp.DimseStatus != STATUS_Success && rsp.DimseStatus != STATUS_Pending)
		{
			...
			if( showErrorMessage == YES && _abortAssociation == NO)
				...
				[[AppController sharedAppController] displayUserNotification: NSLocalizedString(@"Query Failed (2)", nil) description: response name: @"autoquery"];
