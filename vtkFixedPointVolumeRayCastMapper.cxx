...
vtkCxxRevisionMacro(vtkFixedPointVolumeRayCastMapper, "$Revision: 1.46 $");
...
VTK_THREAD_RETURN_TYPE vtkUpdateMinMaxDispatcher( void *arg )
{
  ...
  int components   = mapper->GetCurrentScalars()->GetNumberOfComponents();
  int independent  = mapper->GetVolume()->GetProperty()->GetIndependentComponents();
      ...
      switch ( scalarType )
        ...
        vtkTemplateMacro( 
          vtkFixedPointVolumeRayCastMapperFillInMinMaxVolume(
            (VTK_TT *)(dataPtr), mapper->GetMinMaxVolume(), dim, targetSize, 
            independent, components, thread_id, thread_count, mapper->GetTableShift(), mapper->GetTableScale()) );
        }
...
void vtkFixedPointVolumeRayCastMapper::UpdateMinMaxVolume( vtkVolume *vol )
{
  ...
  if ( needToUpdate&0x02 )
    ...
    if ( this->MinMaxVolumeSize[0] != targetSize[0] ||
	  ...
	  this->Threader->SetSingleMethod( vtkUpdateMinMaxDispatcher, (vtkObject *) this);
