		...
		if( axe == 0)		// X - RESLICE
		{
			...
			if( sign > 0)
			{
				...
				for( y = 0; y < newY; y++)
				{
					...
					dstP = [curPix fImage] + (newY-y-1) * newX;
			...
			else
			{
				...
				for( y = 0; y < newY; y++)
				{
					srcP = [[pixList objectAtIndex: y] fImage] + i * [[pixList objectAtIndex: y] pwidth];
		...
		else									// Y - RESLICE
		{
			...
			if( thickSlab > 1 && Ycache == 0L)
			{
				...
				if( Ycache)
				{
					...
					for ( x = 0; x < newY; x += 2) [unitsSet addObject: [NSNumber numberWithInt:x]];
