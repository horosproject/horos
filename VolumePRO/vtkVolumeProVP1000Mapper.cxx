...
vtkCxxRevisionMacro(vtkVolumeProVP1000Mapper, "$Revision: 1.1.1.1 $");
...
void vtkVolumeProVP1000Mapper::SetMaxWindow( long m)
{
	maxWindow = m;
}

void vtkVolumeProVP1000Mapper::SetRenderFlag( long m)
{
	renderFlag = m;
}

VLIContext* vtkVolumeProVP1000Mapper::GetContextPointer( )
{
	return this->Context;
}
...
vtkVolumeProVP1000Mapper::vtkVolumeProVP1000Mapper()
{
  ...
  if ( status != kVLIOK )
	...
	printf(" VLIVersionString %s\n",      VLIConfiguration().GetVersionString ( VLIConfiguration::kVLIVersion));
	printf(" Driver Version %s\n",      VLIConfiguration().GetVersionString (  VLIConfiguration::kDriverFound));
...
{
	double viewAngleRadians = ren->GetActiveCamera()->GetViewAngle() * M_PI / 180.0;
	...
	projectionMatrixVLI[2][2] = (clippingRange[1] + clippingRange[0]) /(clippingRange[0] - clippingRange[1]);
	projectionMatrixVLI[2][3] = 2.0 * clippingRange[1] * clippingRange[0] /(clippingRange[0] - clippingRange[1]);
	projectionMatrixVLI[3][2] = -1.0;
	projectionMatrixVLI[3][3] = 0.0;
	
//	 vtkErrorMacro( << "clippingRange" << clippingRange[0]);
//		vtkErrorMacro( << "clippingRange"  << clippingRange[1]);
	
//	projectionMatrixVLI = VLIMatrix::Scale(1.0, -1.0, 1.0) * VLIMatrix::Perspective(viewAngleRadians, aspect[0] / aspect[1], clippingRange[0], clippingRange[1]);
...
//	this->Context->SetSamplingFactor( 4.0 );
}
...
void vtkVolumeProVP1000Mapper::SetShading(vtkRenderer *ren, vtkVolume *vol)
{
...
void vtkVolumeProVP1000Mapper::SetLookUpTable( long start, long end, unsigned char	*r, unsigned char	*g, unsigned char	*b, unsigned short *o, vtkRenderer *ren, vtkVolume *vol)
{
	...
	if( start >= 4096) start = 4096;
	
	for ( i = 0; i< start; i++)
	{
		rgbTable[ i][ 0] = rgbTable[ i][ 1] = rgbTable[ i][ 2] = 0;
		aTable[ i] = 0;
	}
	
	for ( i = start; i< end; i++)
	{
		val = ((i - start) * 255) / (end - start);
		val = (val < 0)?(0):(val);
        val = (val > 255)?(255):(val);
	...
	for ( i = end; i< 4096; i++)
	{
		rgbTable[ i][ 0] = rgbTable[ i][ 1] = rgbTable[ i][ 2] = 255;
		aTable[ i] = 4095;
	}
...
void vtkVolumeProVP1000Mapper::UpdateProperties( vtkRenderer *ren, 
                                                vtkVolume *vol )
{
	return;		// EXTREMELY SLOOOOOOW FUNCTION !
...
//  vtkErrorMacro( << "Cropping: " << this->VoxelCroppingRegionPlanes[0] );
...
//this->CroppingRegionFlags = VTK_CROP_CROSS;
...
//          
//          this->VolumeDataType = VTK_VOLUME_16BIT;	BAD QUALITY IF 16 bits???
//          }
//        else
//          {
          this->Volume->SetFieldDescriptor(kVLIField0,
...
//  this->UpdateProperties( ren, vol );  
  ...
  windowSize[0] = maxWindow;
  windowSize[1] = maxWindow;
...
//	  vtkErrorMacro( << "Width: " << width );
//	  vtkErrorMacro( << "Height: " << height );
//	  
//	  vtkErrorMacro( << "imageWidth: " << imageWidth );
//	  vtkErrorMacro( << "imageHeight: " << imageHeight );
...
//	  vtkErrorMacro( << "newWidth: " << newWidth );
//	  vtkErrorMacro( << "newHeight: " << newHeight );
  ...
  if ( ! this->DrawBoundingBox)
    ...
    if ( ! this->IntermixIntersectingGeometry )
      ...
      status = this->Volume->Render(this->Context, this->ImageBuffer, 0, 0, this->DepthBuffer);
	...
	if( outDataSize[0] != this->ImageBuffer->GetWidth() || outDataSize[1] != this->ImageBuffer->GetHeight())
	{
		outDataSize[0] = this->ImageBuffer->GetWidth();
		outDataSize[1] = this->ImageBuffer->GetHeight();
