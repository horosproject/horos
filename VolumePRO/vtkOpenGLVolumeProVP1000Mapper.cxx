...
vtkCxxRevisionMacro(vtkOpenGLVolumeProVP1000Mapper, "$Revision: 1.1.1.1 $");
...
glPixelStorei (GL_UNPACK_CLIENT_STORAGE_APPLE, 1);
  ...
  glTexImage2D( GL_TEXTURE_RECTANGLE_EXT, 0, GL_RGBA8,
                size[0], size[1],
                0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8_REV, outData );
//#else
//  glTexImage2D( GL_TEXTURE_2D, 0, 4,
//                textureSize[0], textureSize[1],
//                0, GL_BGRA_EXT, GL_UNSIGNED_INT_8_8_8_8_REV, textureData );
//#endif 
...
glBindTexture(GL_TEXTURE_RECTANGLE_EXT, texture);
	...
	glBegin (GL_TRIANGLE_STRIP); // draw either tri strips of line strips (so this will drw either two tris or 3 lines)
		...
		glVertex3d (planeCoords[0][0], planeCoords[0][1], planeCoords[0][2]);

		glTexCoord2f (size[0], 0); // draw lower left in world coordinates
		glVertex3d (planeCoords[1][0], planeCoords[1][1], planeCoords[1][2]);

		glTexCoord2f (0, size[1]); // draw upper right in world coordinates
		glVertex3d (planeCoords[3][0], planeCoords[3][1], planeCoords[3][2]);

		glTexCoord2f (size[0], size[1]); // draw lower right in world coordinates
		glVertex3d (planeCoords[2][0], planeCoords[2][1], planeCoords[2][2]);
